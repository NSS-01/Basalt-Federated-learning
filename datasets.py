...
class MNIST_truncated(data.Dataset):
    ...
    def __build_truncated_dataset__(self):
        ...
        mnist_dataobj = MNIST(self.root, train=self.train, transform=self.transform, target_transform=self.target_transform, download=self.download)
    ...
    def truncate_channel(self, index):
        pass  # MNIST是灰度图像，不需要截取通道

    def __getitem__(self, index):
        img, target = self.data[index], int(self.target[index])

        if self.transform is not None:
            img = Image.fromarray(img)#by lhs
            img = self.transform(img)
