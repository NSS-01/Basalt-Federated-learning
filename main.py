...
def train_net(net_id, net, train_dataloader, test_dataloader, epochs, lr, args_optimizer, args, device="cpu"):
    ...
    elif net_id in [i for i in range(14,17)]:
        for epoch in range(epochs):
            epoch_loss_collector = []
            for batch_idx, (x, target) in enumerate(train_dataloader):
    ...
    elif net_id in [i for i in range(17,20)]:
        for epoch in range(epochs):
            epoch_loss_collector = []
            for batch_idx, (x, target) in enumerate(train_dataloader):
                x, target = x.cuda(), target.cuda()
    ...
    train_acc, _ = compute_accuracy(net, train_dataloader, device=device)
    test_acc, conf_matrix, _ = compute_accuracy(net, test_dataloader, get_confusion_matrix=True, device=device)
    print("client" + str(net_id) + "Training accuracy" + str(train_acc))
    print("client" + str(net_id) + "Test accuracy" + str(test_acc))
    logger.info('>> Training accuracy: %f' % train_acc)
    logger.info('>> Test accuracy: %f' % test_acc)
    net.to('cpu')
    ...
    if args.alg == 'moon':
        ...
        for round in range(n_comm_rounds):
            ...
            for net_id, net in enumerate(nets_this_round.values()):
                ...
                # para_vector = torch.nn.utils.parameters_to_vector(net.parameters())
                # print(para_vector)
        ...
        for round in range(n_comm_rounds):
            ...
            # mkdirs(args.modeldir + 'Median/')
            global_model.to('cpu')

            # torch.save(global_model.state_dict(),
            #            args.modeldir + 'Median/' + 'globalmodel' + args.log_file_name + '.pth')
            # torch.save(nets[0].state_dict(), args.modeldir + 'Median/' + 'localmodel0' + args.log_file_name + '.pth')
        ...
        for round in range(n_comm_rounds):
            ...
            # mkdirs(args.modeldir+'trimmed_mean/')
            global_model.to('cpu')
            #
            # torch.save(global_model.state_dict(), args.modeldir+'trimmed_mean/'+'globalmodel'+args.log_file_name+'.pth')
            # torch.save(nets[0].state_dict(), args.modeldir+'trimmed_mean/'+'localmodel0'+args.log_file_name+'.pth')
        ...
        for round in range(n_comm_rounds):
            ...
            logger.info('>> krum Global Model Train accuracy: %f' % train_acc)
            logger.info('>> krum Global Model Test accuracy: %f' % test_acc)
            logger.info('>> krum Global Model Train loss: %f' % train_loss)
            print(" krum Global Model Train accuracy" + str(train_acc))
            print("krum Global Model Test accuracy" + str(test_acc))
            # mkdirs(args.modeldir+'trimmed_mean/')
            global_model.to('cpu')